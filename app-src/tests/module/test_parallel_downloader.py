...
class StripePlanningTest(unittest.TestCase):
    ...
    def test_resume_never_bridges_a_completed_chunk(self):
        ...
        self.assertEqual(
            ...
            [
                [chunk.offset // CHUNK_SIZE for chunk in stripe]
...
class PoolKurigramClient(FakeKurigramClient):
    ...
    async def get_session(
        ...
        **_kwargs,
        ...
        self.get_session_calls.append(
            (dc_id, is_media, is_cdn, temporary)
        )
    ...
    async def test_close_stops_all_temporary_sessions_once(self):
        ...
        self.assertTrue(
            all(
                ...
                for session in client.temporary_sessions
            )
        )
...
class PooledParallelDownloaderTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_retry_keeps_durable_chunks_and_uses_a_fresh_lease(self):
        ...
        self.assertTrue(
            all(
                recorded == durable
                for recorded, durable in pool.committed_observations
            )
        )
    ...
    async def test_pooled_mode_does_not_own_source_lifecycle(self):
        data = b"x" * CHUNK_SIZE
...
class ParallelWriterTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_expected_candidate_and_manifest_inodes_preserve_normal_download(self):
        data = b"abcdefghijkl"
        self.part_path.write_bytes(b"")
        candidate = self.part_path.stat()
        manifest_path = Path(f"{self.part_path}.manifest.sqlite3")
    ...
    async def test_cancellation_waits_for_final_integrity_and_closes_worker_fd_once(self):
        data = b"abcdefghijkl"
        ...
        class DrainingAbortSource:
            ...
            async def iter_range(self, start_offset, expected_length):
                ...
                try:
                    ...
                    if start_offset == 0:
                        yield self.data[
                            start_offset : start_offset + expected_length
                        ]
                    else:
