...
from pyrogram.errors import BadMsgNotification, FloodWait, SecurityError, Unauthorized
...
class DownloadManifest:
    ...
    def completed_chunks(self) -> Dict[int, Tuple[int, str]]:
        ...
        return {
            int(row["offset"]): (int(row["length"]), str(row["sha256"])) for row in rows
        }
...
def _is_fatal_session_error(error: BaseException) -> bool:
    ...
    return any(isinstance(current, fatal_errors) for current in _exception_chain(error))
...
class _SessionBoundClient:
    ...
    async def get_session(
        ...
        **kwargs,
        ...
        cdn_session_getter = getattr(
            self._media_session,
            "get_cdn_session",
            None,
        )
        if is_cdn and cdn_session_getter is not None:
            return await cdn_session_getter(
                dc_id,
                is_media=is_media,
                is_cdn=is_cdn,
                **kwargs,
...
def _verify_manifest_and_hash_sync(
    ...
    local_size = (
        os.fstat(path).st_size if isinstance(path, int) else os.path.getsize(path)
    )
    ...
    covered = 0
    local_file = (
        os.fdopen(os.dup(path), "rb") if isinstance(path, int) else open(path, "rb")
    )
    with local_file:
        local_file.seek(0)
        for offset, (length, expected_digest) in sorted(records.items()):
            if offset != covered:
                raise IncompleteRange(f"manifest gap from {covered} to {offset}")
    ...
    if covered != file_size:
        raise IncompleteRange(f"manifest covers {covered}/{file_size} bytes")
    ...
    with local_file:
        for item in ordered:
            ...
            if len(data) != actual_length:
                raise IncompleteRange(f"local hash range at {item.offset} is short")
            actual_digest = hashlib.sha256(data).digest()
            if not hmac.compare_digest(actual_digest, item.digest):
                raise HashMismatch(f"Telegram hash mismatch at offset {item.offset}")
            covered = max(covered, item.offset + actual_length)

    if covered < file_size:
        raise RemoteHashUnavailable(f"remote hashes cover {covered}/{file_size} bytes")
...
class ParallelDownloader:
    ...
    def __init__(
        ...
        *,
        ...
        pool=None,
        coordinator: Optional[MediaTransferCoordinator] = None,
        file_pool_config: Optional[FilePoolConfig] = None,
        ...
        if pool is not None and coordinator is not None:
            raise ValueError("pool and coordinator are mutually exclusive")
        if coordinator is not None and chunk_size != CHUNK_SIZE:
            raise ValueError("per-file downloads require 1 MiB manifest chunks")
        if stripe_size < CHUNK_SIZE or stripe_size % CHUNK_SIZE:
            raise ValueError("stripe_size must be a positive 1 MiB multiple")
        ...
        self.file_pool_config = (
            file_pool_config or FilePoolConfig()
            if coordinator is not None
            else file_pool_config
        )
            ...
            try:
                async with lease:
                    ...
                    try:
                        ...
                        elif _find_flood_wait(error) is None and is_retryable_timeout(
                            error
                        ):
                ...
                if isinstance(
                    error, (_InjectedAbortRequested, HashMismatch)
    ...
    async def _download_file_stripe(
        self,
        session,
        work: _FileStripeWork,
        fd: int,
        manifest: DownloadManifest,
        file_size: int,
        progress,
        file_pool: FileMediaSessionPool,
    ) -> None:
        pending = work.chunks[work.next_index :]
        if not pending:
            return
        start_offset = pending[0].offset
        expected_length = sum(chunk.length for chunk in pending)
        ...
        try:
            async for data in iterator:
                ...
                chunk = work.chunks[work.next_index]
                if len(data) != chunk.length:
                    raise IncompleteRange(
                        f"chunk at {chunk.offset} expected {chunk.length} "
                        f"bytes, got {len(data)}"
                ...
                file_pool.record_committed(chunk.length)
                work.next_index += 1
                self._downloaded_chunks += 1
                self._completed_bytes += chunk.length
                await self._notify_progress(progress, file_size)
                if (
                    ...
                    raise _InjectedAbortRequested("parallel download abort requested")
            ...
            flood_wait = _find_flood_wait(error)
            if work.done and (flood_wait is not None or is_retryable_timeout(error)):
                return
            if flood_wait is not None:
                raise StripeAttemptError(
                    error,
                    "flood_wait",
                    wait_seconds=max(float(flood_wait.value), 0.0),
                ) from error
            if is_retryable_timeout(error):
    ...
    async def download(
        ...
        *,
        ...
        if self.pool is None and self.coordinator is None and prepare is not None:
        ...
        try:
            ...
            if close is not None:
                try:
                    ...
                    if primary_error is None:
                        raise
                    logger.exception("Failed to close parallel download range source")
        ...
        try:
            ...
            try:
                if self.coordinator is not None:
                    if self.file_pool_config is None:
                        raise AssertionError("per-file pool config is missing")
                    transfer_id = self.transfer_id or identity.stable_key()
                    works = [
                        _FileStripeWork(stripe)
                    ...
                    async def download_stripe(session, work):
                        await self._download_file_stripe(
                            session,
                            work,
                            fd,
                            manifest,
                    ...
                    try:
                        await file_pool.run(works, download_stripe)
                    finally:
                        self._retries += file_pool.snapshot().retries
            ...
            actual = {
                offset: length for offset, (length, _digest) in final_records.items()
            }
            if actual != expected:
