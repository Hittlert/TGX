...
from module.tdl_client import TDLClient
...
class Application:
    ...
    def __init__(
        ...
        self.download_consumer_enabled: bool = True
        self.download_producer_enabled: bool = True
        self.download_backend: str = "python"
        self.tdl_api_url: str = "http://tdl-daemon:18080"
        self.tdl_poll_interval: float = 1.0
        self.tdl_inflight_task_limit: int = 16
        self.tdl_client: Optional[TDLClient] = None
    ...
    @staticmethod
    def normalize_target_priority(priority: Any) -> int:
        """Normalize persisted target priority without breaking old configs."""
        try:
            priority_int = int(priority or 0)
        except (TypeError, ValueError):
            return 0
        return max(-1000000, min(1000000, priority_int))
    ...
    def get_media_pool_status(self) -> dict:
        ...
        if getattr(self, "download_backend", "python") == "tdl":
            return self._get_tdl_pool_status()
        ...
        if mode == "per_file" and coordinator is not None:
            ...
            return {
                ...
                "committed_bytes_per_second": (snapshot.committed_bytes_per_second),
        ...
        payload["draining"] = 0
        payload["raw_bps"] = float(payload.get("committed_bytes_per_second", 0) or 0)
    ...
    def _get_tdl_pool_status(self) -> dict:
        """Map daemon metrics onto the existing Web status contract."""
        try:
            status = self.tdl_client.get_status() if self.tdl_client else {}
        except Exception as error:
            status = {"online": False, "last_error": str(error)}

        online = bool(status.get("online", False))
        speed = float(status.get("rolling_5s_bps", 0) or 0) if online else 0.0
        active_items = status.get("active_files", [])
        ...
        for item in active_items:
            if not isinstance(item, dict):
                continue
            task_id = str(
                item.get("id") or f"{item.get('peer', '')}:{item.get('message_id', '')}"
            )
            files[task_id] = _thaw_snapshot(item)

        pool = status.get("pool", {})
        if not isinstance(pool, dict):
            pool = {}
        pool_size = int(pool.get("size", 0) or 0) if online else 0
        active_count = len(files) if online else 0
        return {
            "enabled": online,
            "mode": "tdl",
            "online": online,
            ...
            "desired": pool_size,
            "live": pool_size,
            "idle": max(pool_size - active_count, 0),
            "created": int(pool.get("created", 0) or 0),
            "reused": 0,
            "active_slots": active_count,
            "hard_limit": pool_size,
            "pipeline_depth": 0,
            "last_scale_reason": "tdl" if online else "offline",
            "used": pool_size,
            "creating": 0,
            "draining": 0,
            "active_files": active_count,
            "raw_bps": speed,
            "rolling_5s_bps": speed,
            "p10_5s_bps": speed,
            "mean_5s_bps": speed,
            "stddev_5s_bps": 0.0,
            "cv": 0.0,
            ...
            "raw_samples": [],
            "rolling_5s_samples": [],
            "committed_bytes_per_second": speed,
            "expansion_queue": 0,
            "queue_depth": int(status.get("queue_depth", 0) or 0),
            "dc_cooldowns": {},
            "fallbacks": 0,
            "retries": int(status.get("retries", 0) or 0),
            "resets": int(pool.get("reconnects", 0) or 0),
            "reconnects": int(pool.get("reconnects", 0) or 0),
            "last_error": str(status.get("last_error", "") or ""),
            "updated_at": status.get("updated_at", 0),
            "pool": _thaw_snapshot(pool),
            "files": files if online else {},
        }
        ...
        with self.config_lock:
            ...
            for item in self.config.get("chat", []) or []:
                ...
                target = {
                    ...
                    "priority": self.normalize_target_priority(
                        item.get("priority", 0)
                    ),
    ...
    def save_listen_targets(
        ...
        with self.config_lock:
            ...
            for item in targets:
                ...
                priority = self.normalize_target_priority(item.get("priority", 0))
                if priority:
                    chat_item["priority"] = priority
                ...
                if (
                    ...
                    scan_status_by_chat[chat_key] = scan_status_by_chat[source_chat_key]
            ...
            download_records = self.ensure_download_records()
            download_records.skip_unmonitored_targets(list(chat_config))
            for chat_id, chat_download_config in chat_config.items():
                download_records.override_cursor(
    ...
    def assign_config(self, _config: dict) -> bool:
        ...
        self.download_consumer_enabled = get_config(
            _config,
            "download_consumer_enabled",
            self.download_consumer_enabled,
            bool,
        )
        self.download_producer_enabled = get_config(
            _config,
            "download_producer_enabled",
            self.download_producer_enabled,
            bool,
        )

        configured_backend = (
            str(_config.get("download_backend", "python")).strip().lower()
        )
        if configured_backend not in ("python", "tdl"):
            raise ValueError("download_backend must be either 'python' or 'tdl'")
        self.download_backend = configured_backend
        self.tdl_api_url = str(
            _config.get("tdl_api_url", "http://tdl-daemon:18080")
        ).rstrip("/")
        if not self.tdl_api_url:
            self.tdl_api_url = "http://tdl-daemon:18080"
        try:
            configured_poll_interval = float(_config.get("tdl_poll_interval", 1.0))
        except (TypeError, ValueError):
            configured_poll_interval = 1.0
        self.tdl_poll_interval = (
            configured_poll_interval if 0.1 <= configured_poll_interval <= 30.0 else 1.0
        )
        try:
            configured_inflight_limit = int(
                _config.get("tdl_inflight_task_limit", 16)
            )
        except (TypeError, ValueError):
            configured_inflight_limit = 16
        self.tdl_inflight_task_limit = (
            configured_inflight_limit
            if 1 <= configured_inflight_limit <= 64
            else 16
        )
        self.tdl_client = (
            TDLClient(self.tdl_api_url) if self.download_backend == "tdl" else None
        )
        ...
        self.parallel_download_workers = (
            configured_parallel_workers if 1 <= configured_parallel_workers <= 4 else 2
        )
        ...
        supported_file_pool_stripes = {size * 1024 * 1024 for size in (5, 10, 20)}
    ...
    def recover_interrupted_downloads(self) -> int:
        """Recover legacy worker rows only when this process owns consumption."""
        if not self.download_consumer_enabled:
            return 0
        return self.ensure_download_records().recover_processing()
    ...
    def record_scanned_message(self, chat_id: Union[int, str], message_id: int) -> int:
    ...
    def claim_download_record(self) -> Optional[Dict[str, Any]]:
        ...
        with self.consumer_claim_lock:
            ...
            for offset in range(len(chat_ids)):
                ...
                record = self.ensure_download_records().claim_next([chat_ids[index]])
