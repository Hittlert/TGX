...
def claim_if_downloading(
    claim: Callable[[], ClaimValue],
) -> Tuple[bool, Optional[ClaimValue]]:
    """Run a synchronous task claim atomically with the pause transition."""
    with _download_state_lock:
        if _download_state is DownloadState.StopDownload:
            return False, None
        return True, claim()
