...
class TDLClient:
    ...
    def get_control_targets(self) -> List[Dict[str, Any]]:
        """Return durable Go control-plane targets."""
        payload = self._request_json(
            "GET", "/api/v1/targets", timeout=self.request_timeout
        )
        targets = payload.get("targets")
        if not isinstance(targets, list):
            raise TDLUnavailable("tdl returned invalid control targets")
        return [dict(item) for item in targets if isinstance(item, dict)]

    def get_target_progress(self) -> List[Dict[str, Any]]:
        """Return read-only per-target ledger progress."""
        payload = self._request_json(
            "GET", "/api/v1/target-progress", timeout=self.request_timeout
        )
        progress = payload.get("progress")
        if not isinstance(progress, list):
            raise TDLUnavailable("tdl returned invalid target progress")
        return [dict(item) for item in progress if isinstance(item, dict)]

    def refresh_dialogs(self) -> Dict[str, Any]:
        """Refresh the Go control-plane dialog cache."""
        return self._request_json(
            "POST",
            "/api/v1/dialogs/refresh",
            timeout=max(self.request_timeout * 5, 10.0),
        )

    def update_control_target(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Apply one partial target update to the durable Go control plane."""
        return self._request_json(
            "PUT",
            "/api/v1/targets",
            json=dict(payload),
...
def _copy_status(status: Dict[str, Any]) -> Dict[str, Any]:
    copy = dict(status)
    copy["active_files"] = [
        dict(item) for item in (status.get("active_files") or [])
    ]
