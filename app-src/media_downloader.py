...
def _download_worker_count() -> int:
    """Return the bounded in-flight window for the active backend."""
    if not app.download_consumer_enabled:
        return 0
    if app.download_backend == "tdl":
        return app.tdl_inflight_task_limit
    return app.max_download_task
...
def _should_use_parallel_download(media_size: int) -> bool:
    ...
    return bool(
        app.parallel_download_enabled and media_size >= app.parallel_download_min_size
    )


def _should_use_global_pool(media_size: int) -> bool:
    """Return whether this file should use the process-wide session pool."""
    return bool(
...
def _should_use_per_file_pool(media_size: int) -> bool:
    """Return whether this file should use its own media-session pool."""
    return bool(
        app.parallel_pool_mode == "per_file"
        and app.media_transfer_coordinator is not None
        and media_size > app.parallel_file_pool_threshold
    )
...
async def _download_to_temp(
    ...
    global_pool = app.media_session_pool
    coordinator = app.media_transfer_coordinator
    use_global_pool = _should_use_global_pool(media_size)
    use_per_file_pool = _should_use_per_file_pool(media_size)
    if app.parallel_pool_mode == "per_file":
        if not use_per_file_pool:
            return await sequential_download()
    elif app.parallel_session_pool_enabled:
        if not use_global_pool:
            return await sequential_download()
    elif not _should_use_parallel_download(media_size):
        return await sequential_download()
    ...
    try:
        ...
        if use_per_file_pool:
            file_pool_config = FilePoolConfig(
                initial_sessions=app.parallel_file_pool_initial_sessions,
                max_sessions=app.parallel_file_pool_max_sessions,
                control_interval=app.parallel_file_pool_control_interval,
                growth_hold=app.parallel_file_pool_growth_hold,
            )
            downloader = ParallelDownloader(
                source,
                workers=app.parallel_download_workers,
                coordinator=coordinator,
                file_pool_config=file_pool_config,
                stripe_size=app.parallel_file_pool_stripe_size,
            ...
            downloader = ParallelDownloader(
                ...
                workers=app.parallel_download_workers,
                pool=global_pool,
                stripe_size=app.parallel_pool_stripe_size,
                transfer_id=transfer_id,
            )
        else:
            downloader = ParallelDownloader(
        ...
        if use_global_pool:
            global_pool.record_fallback()
        elif use_per_file_pool:
...
async def download_task(
    ...
    message: pyrogram.types.Message,
    node: TaskNode,
    database_record: Optional[dict] = None,
):
...
async def consume_database_task(client: pyrogram.Client, record: dict) -> bool:
    ...
    try:
        ...
        node.total_task += 1
        node.download_status[message.id] = DownloadStatus.Downloading
        await download_task(client, message, node, database_record=record)
        return True
    except asyncio.CancelledError:
        app.record_download_pending(chat_id, message_id)
        raise
    except Exception as error:
        logger.exception(
...
async def _download_media_with_tdl(
    message: pyrogram.types.Message,
    node: TaskNode,
    file_name: str,
    media_size: int,
    ...
    """Delegate one configured-chat attachment to the project-local tdl daemon."""
    if app.tdl_client is None:
        logger.error("tdl backend is enabled but no daemon client is configured")
        return DownloadStatus.FailedDownload, None

    save_root = os.path.abspath(app.save_path)
    final_path = os.path.abspath(file_name)
    try:
        inside_save_root = os.path.commonpath((save_root, final_path)) == save_root
    except ValueError:
        inside_save_root = False
    if not inside_save_root:
        logger.error("Refusing tdl destination outside save_path: {}", file_name)
        return DownloadStatus.FailedDownload, None

    attempts = int((database_record or {}).get("attempts", 0) or 0)
    task_id = f"{app._chat_id_key(node.chat_id)}:{message.id}"
    payload = {
        "id": task_id,
        "peer": app._chat_id_key(node.chat_id),
        "message_id": int(message.id),
        "final_path": os.path.relpath(final_path, save_root).replace("\\", "/"),
        "expected_size": int(media_size or 0),
        "retry": attempts > 1,
    }

    try:
        task = await app.tdl_client.wait_for_completion(
            payload,
            should_continue=lambda: (app.is_running and not node.is_stop_transmission),
            poll_interval=app.tdl_poll_interval,
        )
    except asyncio.CancelledError:
        raise
    except Exception as error:
        logger.error("tdl task {} failed: {}", task_id, error)
        return DownloadStatus.FailedDownload, None

    state = str(task.get("state", ""))
    if state == "unavailable":
        logger.warning(
            "tdl message {} is unavailable: {}", task_id, task.get("error", "")
        )
        return DownloadStatus.SkipDownload, None
    if state != "success":
        logger.error("tdl task {} failed: {}", task_id, task.get("error", state))
        return DownloadStatus.FailedDownload, None

    expected_size = int(media_size or task.get("total_size", 0) or 0)
    if not os.path.isfile(final_path):
        logger.error("tdl task {} reported success without a published file", task_id)
        return DownloadStatus.FailedDownload, None
    actual_size = os.path.getsize(final_path)
    if expected_size and actual_size != expected_size:
        logger.error(
            "tdl task {} published the wrong size: expected={}, actual={}",
            task_id,
            expected_size,
            actual_size,
        )
    ...
    return DownloadStatus.SuccessDownload, final_path
...
async def download_media(
    ...
    if app.download_backend == "tdl" and not node.bot:
        return await _download_media_with_tdl(
            message,
            node,
            file_name,
            media_size,
...
async def worker(client: pyrogram.client.Client):
    ...
    while app.is_running:
        ...
        try:
            def claim_next():
                try:
                    return "memory", queue.get_nowait()
                except asyncio.QueueEmpty:
                    return "database", app.claim_download_record()

            admitted, claimed = download_stat.claim_if_downloading(claim_next)
            if not admitted:
...
async def wait_without_python_producer():
    """Keep the Web runtime alive while Go owns message production."""
    logger.info("Python configured-chat producer is disabled")
    while app.is_running and not app.restart_program:
        await asyncio.sleep(1)
...
def _exec_loop(client: pyrogram.Client):
    """Exec loop"""
    if app.download_producer_enabled:
        runtime = monitor_downloads(client)
    else:
        runtime = wait_without_python_producer()
    app.loop.run_until_complete(runtime)
...
async def _start_runtime(client):
    """Start the client and publish the configured media-session runtime."""
    app.media_session_pool = None
    app.media_transfer_coordinator = None
    ...
    if app.parallel_pool_mode == "per_file":
        coordinator = MediaTransferCoordinator(
            ...
            CoordinatorConfig(
                ...
                warm_session_limit=min(20, app.parallel_media_session_budget),
                warm_session_ttl=120,
            ),
        )
    ...
    if not app.parallel_session_pool_enabled:
        return None
...
async def _shutdown_runtime(client, tasks, runtime_resource, stop_bot=None) -> None:
    """Stop runtime work before closing media sessions and Telegram client."""
    ...
    cleanup_task = asyncio.create_task(finish_shutdown())
    cancellation = None
    while True:
        try:
...
def main():
    ...
    try:
        ...
        runtime_resource = app.loop.run_until_complete(_start_runtime(client))
        logger.success(_t("Successfully started (Press Ctrl+C to stop)"))

        for _ in range(_download_worker_count()):
            task = app.loop.create_task(worker(client))
            tasks.append(task)

        if app.bot_token and app.download_consumer_enabled:
        ...
        app.loop.run_until_complete(
            _shutdown_runtime(client, tasks, runtime_resource, bot_stopper)
        )
