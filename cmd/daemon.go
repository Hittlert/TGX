...
func NewDaemon() *cobra.Command {
	...
	cmd.Flags().StringVar(&opts.BufferType, "buffer-type", opts.BufferType, "staging buffer type: memory (tmpfs 512MB), ssd (SSD 5GB), none (direct target)")
	cmd.Flags().StringVar(&opts.BufferDir, "buffer-dir", opts.BufferDir, "staging buffer directory")
	cmd.Flags().Int64Var(&opts.BufferSize, "buffer-size", opts.BufferSize, "maximum buffer capacity in bytes (0 uses mode default)")
