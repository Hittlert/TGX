...
def get_total_download_speed() -> int:
    ...
    now = time.time()
    if not _has_recent_active_download(now):
        _total_download_speed = 0
        return 0

    if now - _last_download_time > DOWNLOAD_SPEED_STALE_SECONDS:
        _total_download_speed = 0
        return 0
...
def _has_recent_active_download(now: float) -> bool:
    """Return whether any unfinished download reported progress recently."""
    for messages in _download_result.values():
        for value in messages.values():
            ...
            end_time = float(value.get("end_time", 0) or 0)
            if (
                total_size > 0
                and down_byte < total_size
                and now - end_time <= DOWNLOAD_SPEED_STALE_SECONDS
            ):
                return True
