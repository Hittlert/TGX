...
func (r *Registry) Status() StatusSnapshot {
	...
	if currentBPS > 0 {
		...
		r.smoothedSpeed = 0.5 * r.smoothedSpeed
		if r.smoothedSpeed < 1024 {
			r.smoothedSpeed = 0
		}
	}

	return StatusSnapshot{
		Backend: "tdl", Paused: r.paused,
...
func (r *Registry) snapshotTaskLocked(state *taskState, now time.Time) TaskSnapshot {
	...
	if currentTaskBPS > 0 {
		...
		state.smoothedSpeed = 0.5 * state.smoothedSpeed
		if state.smoothedSpeed < 512 {
