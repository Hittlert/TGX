...
class MobileRuntimeStatusMarkupTestCase(unittest.TestCase):
    def setUp(self):
    ...
    def test_footer_keeps_speed_and_pool_counts_in_wrapping_runtime_line(self):
        html = web.get_flask_app().test_client().get("/").get_data(as_text=True)

        self.assertIn('class="footer-runtime"', html)
        self.assertIn('id="download_speed_title"', html)
        self.assertIn('id="media_pool_status"', html)
        self.assertIn("Pools ' + (pool.active_files", html)
        self.assertIn("Sessions ' + (pool.used", html)
