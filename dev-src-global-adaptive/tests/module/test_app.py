...
class AppTestCase(unittest.TestCase):
    ...
    def test_per_file_pool_defaults_are_bounded(self):
        app = Application("", "")
    ...
    def test_legacy_enabled_flag_maps_to_global_mode(self):
        app = Application("", "")
        ...
        self.assertEqual("global", app.parallel_pool_mode)

    def test_explicit_per_file_mode_wins_over_legacy_flag(self):
        app = Application("", "")
        ...
        app.assign_config(
            {
                ...
                "media_types": [],
                "file_formats": {},
                "parallel_session_pool_enabled": False,
                "parallel_pool_mode": "per_file",
            }
        )

        self.assertEqual("per_file", app.parallel_pool_mode)

    def test_invalid_per_file_values_and_mode_fall_back_to_defaults(self):
        app = Application("", "")
        ...
        app.assign_config(
            {
                ...
                "media_types": [],
                "file_formats": {},
                "parallel_pool_mode": "unknown",
                "parallel_file_pool_threshold": 1024,
                "parallel_file_pool_stripe_size": 3 * 1024 * 1024,
                "parallel_file_pool_initial_sessions": 8,
                "parallel_file_pool_max_sessions": 16,
                "parallel_file_pool_control_interval": 0,
                "parallel_file_pool_growth_hold": 0,
                "parallel_media_session_budget": 61,
                "parallel_file_pool_pipeline_depth": 2,
            }
        )
    ...
    def test_per_file_pool_accepts_supported_stripe_sizes(self):
        mib = 1024 * 1024
        for stripe_size in (5 * mib, 10 * mib, 20 * mib):
            with self.subTest(stripe_size=stripe_size):
                app = Application("", "")
                self.addCleanup(app.loop.close)
                app.assign_config(
                    {
                        "api_id": 123,
                        "api_hash": "hash",
                        ...
                        "file_formats": {},
                        "parallel_pool_mode": "per_file",
                        "parallel_file_pool_stripe_size": stripe_size,
                    }
                )

                self.assertEqual(stripe_size, app.parallel_file_pool_stripe_size)

    def test_per_file_pool_rejects_unsupported_stripe_size(self):
        app = Application("", "")
        self.addCleanup(app.loop.close)
        app.assign_config(
            {
        ...
        app.assign_config(
            {
                ...
                "file_formats": {},
                "parallel_session_pool_enabled": True,
                "parallel_pool_file_threshold": 5 * 1024 * 1024,
                "parallel_pool_stripe_size": 5 * 1024 * 1024,
    ...
    def test_pool_stripe_size_falls_back_unless_exactly_five_mib(self):
        for stripe_size in (1024 * 1024, 10 * 1024 * 1024):
            with self.subTest(stripe_size=stripe_size):
                app = Application("", "")
                self.addCleanup(app.loop.close)
                app.assign_config(
                    {
                        ...
                        "media_types": [],
                        "file_formats": {},
                        "parallel_pool_stripe_size": stripe_size,
                    }
                )

                self.assertEqual(5 * 1024 * 1024, app.parallel_pool_stripe_size)

    def test_pool_file_threshold_falls_back_unless_exactly_five_mib(self):
        for threshold in (
            1024 * 1024,
            5 * 1024 * 1024 - 1,
            5 * 1024 * 1024 + 1,
                ...
                app.assign_config(
                    {
                        ...
                        "media_types": [],
                        "file_formats": {},
                        "parallel_pool_file_threshold": threshold,
                    }
                )
