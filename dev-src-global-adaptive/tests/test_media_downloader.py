...
class DownloadRoutingTest(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        self.original_pool_values = {
            "parallel_pool_mode": app.parallel_pool_mode,
            ...
            "parallel_file_pool_threshold": app.parallel_file_pool_threshold,
            "parallel_file_pool_stripe_size": app.parallel_file_pool_stripe_size,
            "parallel_file_pool_initial_sessions": (
                app.parallel_file_pool_initial_sessions
            ),
            "parallel_file_pool_max_sessions": app.parallel_file_pool_max_sessions,
            "parallel_file_pool_control_interval": (
                app.parallel_file_pool_control_interval
            ),
            "parallel_file_pool_growth_hold": app.parallel_file_pool_growth_hold,
            "media_transfer_coordinator": app.media_transfer_coordinator,
        }
        app.parallel_pool_mode = "off"
    ...
    def test_per_file_pool_threshold_is_strictly_greater_than_five_mib(self):
        app.parallel_pool_mode = "per_file"
        app.parallel_file_pool_threshold = 5 * 1024 * 1024
        app.media_transfer_coordinator = mock.Mock()

        self.assertFalse(_should_use_per_file_pool(5 * 1024 * 1024))
        self.assertTrue(_should_use_per_file_pool(5 * 1024 * 1024 + 1))

    def test_per_file_pool_requires_mode_and_runtime_coordinator(self):
        app.parallel_file_pool_threshold = 1024
        app.media_transfer_coordinator = mock.Mock()
        self.assertFalse(_should_use_per_file_pool(2048))
    ...
    @mock.patch("media_downloader.ParallelDownloader")
    @mock.patch("media_downloader.KurigramRangeSource")
    async def test_large_file_uses_dedicated_file_pool(
        self,
        source_factory,
        downloader_factory,
    ):
        app.parallel_pool_mode = "per_file"
        app.parallel_file_pool_threshold = 5 * 1024 * 1024
        app.parallel_file_pool_stripe_size = 5 * 1024 * 1024
        app.parallel_file_pool_initial_sessions = 4
        app.parallel_file_pool_max_sessions = 12
        app.parallel_file_pool_control_interval = 10
        app.parallel_file_pool_growth_hold = 120
        app.media_transfer_coordinator = mock.Mock()
        self.media.file_size = 5 * 1024 * 1024 + 1
        downloader_factory.return_value.download = mock.AsyncMock(
            return_value=SimpleNamespace(
                path="/tmp/per-file.parallel.part",
                workers=4,
                retries=0,
                sha256="file-pool-sha",
            )
        )
        ...
        self.assertEqual("/tmp/per-file.parallel.part", result)
        kwargs = downloader_factory.call_args.kwargs
        self.assertIs(app.media_transfer_coordinator, kwargs["coordinator"])
        self.assertEqual(5 * 1024 * 1024, kwargs["stripe_size"])
        self.assertEqual("-100123:42:route-test", kwargs["transfer_id"])
        config = kwargs["file_pool_config"]
        self.assertEqual(4, config.initial_sessions)
        self.assertEqual(12, config.max_sessions)
        self.assertEqual(10, config.control_interval)
        self.assertEqual(120, config.growth_hold)
        self.client.download_media.assert_not_awaited()

    async def test_five_mib_file_stays_sequential_in_per_file_mode(self):
        app.parallel_pool_mode = "per_file"
        app.parallel_file_pool_threshold = 5 * 1024 * 1024
        app.media_transfer_coordinator = mock.Mock()
        self.media.file_size = 5 * 1024 * 1024
        ...
        self.assertEqual("/tmp/sequential", result)
        app.media_transfer_coordinator.record_fallback.assert_not_called()
        self.client.download_media.assert_awaited_once()

    @mock.patch("media_downloader.ParallelDownloader")
    @mock.patch("media_downloader.KurigramRangeSource")
    async def test_per_file_failure_falls_back_to_sequential_path(
        self,
        _source_factory,
        downloader_factory,
    ):
        app.parallel_pool_mode = "per_file"
        app.parallel_file_pool_threshold = 1
        app.media_transfer_coordinator = mock.Mock()
        downloader_factory.return_value.download = mock.AsyncMock(
        ...
        self.assertEqual("/tmp/sequential", result)
        app.media_transfer_coordinator.record_fallback.assert_called_once_with()
        self.client.download_media.assert_awaited_once()

    @mock.patch("media_downloader.ParallelDownloader")
    @mock.patch("media_downloader.KurigramRangeSource")
    async def test_per_file_cancellation_does_not_start_fallback(
        self,
        _source_factory,
        downloader_factory,
    ):
...
class RuntimeShutdownTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.original_coordinator = app.media_transfer_coordinator
        self.original_mode = app.parallel_pool_mode
        self.original_enabled = app.parallel_session_pool_enabled

    def tearDown(self):
        app.media_session_pool = self.original_pool
        app.media_transfer_coordinator = self.original_coordinator
        app.parallel_pool_mode = self.original_mode
        app.parallel_session_pool_enabled = self.original_enabled

    @mock.patch("media_downloader.start_server", new_callable=mock.AsyncMock)
    @mock.patch("media_downloader.MediaTransferCoordinator")
    @mock.patch("media_downloader.KurigramMediaSessionFactory")
    async def test_start_runtime_publishes_per_file_coordinator(
        self,
        session_factory,
        coordinator_factory,
        start_server_mock,
        ...
        client = mock.Mock()
        coordinator = SimpleNamespace(start=mock.Mock(), close=mock.AsyncMock())
        coordinator_factory.return_value = coordinator
        session_factory.return_value = "session-factory"
        app.parallel_pool_mode = "per_file"
        app.parallel_session_pool_enabled = False
        app.parallel_media_session_budget = 60
        app.parallel_file_pool_control_interval = 10

        result = await _start_runtime(client)

        self.assertIs(coordinator, result)
        self.assertIs(coordinator, app.media_transfer_coordinator)
        self.assertIsNone(app.media_session_pool)
        start_server_mock.assert_awaited_once_with(client)
        coordinator_factory.assert_called_once()
        coordinator.start.assert_called_once_with()
        self.assertEqual("session-factory", coordinator_factory.call_args.args[0])
        config = coordinator_factory.call_args.args[1]
        self.assertEqual(60, config.max_sessions)
        self.assertEqual(10, config.expansion_interval)
        self.assertTrue(config.warm_session_handoff)
        self.assertEqual(20, config.warm_session_limit)
        self.assertEqual(120, config.warm_session_ttl)

    @mock.patch("media_downloader.start_server", new_callable=mock.AsyncMock)
    @mock.patch("media_downloader.MediaTransferCoordinator")
    async def test_per_file_start_failure_closes_coordinator_and_clears_handle(
        self,
        coordinator_factory,
        _start_server_mock,
    ):
        failure = RuntimeError("coordinator start failed")
        coordinator = SimpleNamespace(
            start=mock.Mock(side_effect=failure),
        ...
        with self.assertRaises(RuntimeError) as raised:
            await _start_runtime(mock.Mock())

        self.assertIs(failure, raised.exception)
        coordinator.close.assert_awaited_once_with()
        self.assertIsNone(app.media_transfer_coordinator)

    async def test_shutdown_closes_coordinator_before_client_and_clears_handles(self):
        events = []
        coordinator = SimpleNamespace(
            close=mock.AsyncMock(side_effect=lambda: events.append("coordinator"))
        )
        ...
        await _shutdown_runtime(client, [], coordinator)

        self.assertEqual(["coordinator", "client"], events)
        self.assertIsNone(app.media_transfer_coordinator)
        self.assertIsNone(app.media_session_pool)
...
class RuntimeMainTest(unittest.TestCase):
    ...
    def test_main_starts_configured_pool_after_client_before_workers(self):
        ...
        pool_factory = mock.Mock(
            side_effect=lambda factory, config: (events.append("pool-create") or pool)
        )
...
@mock.patch("media_downloader.check_for_updates", new=check_for_updates, create=True)
