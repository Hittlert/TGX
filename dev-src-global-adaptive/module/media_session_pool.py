...
class _PersistentSessionView:
    """Delegate requests while leaving lifecycle ownership with its media lane."""
    ...
    async def invoke(self, request, **kwargs):
        kwargs.setdefault("retries", FILE_RPC_RETRIES)
        kwargs.setdefault("timeout", FILE_RPC_TIMEOUT)
        return await self._session.invoke(request, **kwargs)
...
class ReusableMediaSession:
    """Own one media transport and its lazily reused CDN transports."""

    def __init__(self, client, session):
        self.client = client
        self.raw_session = session
        self._cdn_lock = asyncio.Lock()
        self._cdn_sessions: Dict[Tuple[Any, ...], Tuple[Any, Any]] = {}
        self._closed = False
    ...
    async def invoke(self, request, **kwargs):
        kwargs.setdefault("retries", FILE_RPC_RETRIES)
        kwargs.setdefault("timeout", FILE_RPC_TIMEOUT)
        return await self.raw_session.invoke(request, **kwargs)

    async def get_cdn_session(
        self,
        dc_id=None,
        is_media=False,
        is_cdn=False,
        **kwargs,
        ...
        if not is_cdn:
            raise ValueError("persistent auxiliary sessions must be CDN sessions")
        key = (
            dc_id,
            kwargs.get("server_address"),
            kwargs.get("port"),
        )
        async with self._cdn_lock:
            if self._closed:
                raise RuntimeError("media session is already closed")
            existing = self._cdn_sessions.get(key)
            ...
            view = _PersistentSessionView(session)
            self._cdn_sessions[key] = (session, view)
            return view
    ...
    async def _stop_all(self) -> None:
        async with self._cdn_lock:
            self._closed = True
            sessions = [
                session for session, _view in self._cdn_sessions.values()
            ]
            self._cdn_sessions.clear()
        sessions.append(self.raw_session)
        ...
        async with self.lock:
            for _attempt in range(self.attempts):
                try:
                    ...
                    return ReusableMediaSession(self.client, session)
