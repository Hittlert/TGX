        ...
        try:
            ...
            try:
                if monitor_task:
                    monitor_task.cancel()
                tasks = [
                    task for task in (reply_task, monitor_task) if task is not None
                ]
                if tasks:
                    await asyncio.gather(*tasks, return_exceptions=True)
            finally:
                if _bot.reply_task is reply_task:
                    _bot.reply_task = None
                if _bot.monitor_task is monitor_task:
                    _bot.monitor_task = None
        if failure is not None:
            raise failure

    cleanup_task = asyncio.create_task(finish_stop())
    cancellation = None
    while True:
        try:
