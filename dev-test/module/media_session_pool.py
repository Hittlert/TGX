...
class GlobalMediaSessionPool:
    ...
    async def _control_once(self) -> None:
        ...
        async with self._lock:
            ...
            self._desired = min(
                self._config.max_sessions,
                max(decision.target, active_sessions),
            )
            ...
            retire.extend(expired)
            if decision.reason != "idle":
                excess = max(0, len(self._entries) - self._desired)
                for entry in idle:
                    if len(retire) >= excess:
                        break
                    if entry not in retire:
                        retire.append(entry)
            for entry in retire:
                entry.retiring = True
                entry.stopping = True
    ...
    def _ensure_builder_locked(self, dc_id: int) -> None:
        ...
        if owned_or_creating >= capacity_limit:
            ...
            idle_entries = [
                entry
                for entry in self._entries
                if entry.active_slots == 0 and not entry.retiring
            ]
            if idle_entries:
                ...
                active_entries = [
                    entry for entry in self._entries if not entry.retiring
                ]
                if active_entries:
